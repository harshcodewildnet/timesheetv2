...
<body>
            ...
            <div class="left">
                ...
                <div class="filter-multiselect">
                    ...
                    <div class="options">
                        ...
                        <label><input type="checkbox" class="filter-option timeline-filter" value="last7" checked> Last 7
                            Days</label>
            ...
            <div class="tab-container">
                <div class="tab-links" role="tablist">
                    ...
                    <button class="tab-link" role="tab" aria-selected="false" data-tab="special">Special
                        Cases</button>
                    <button class="tab-link" role="tab" aria-selected="false" data-tab="utilization">Utilization</button>
                </div>
                <div class="tab-content">
                                ...
                                <table id="tasktable">
                                    <thead id="tasktablehead">
                                        <tr>
                                            ...
                                            <th>Date</th>
                                            <th class="short">Emp Id</th>
                                            <th>Name</th>
                                            <th>Working/Leave</th>
                                            <th class="short">Duration</th>
                    ...
                    <div class="tab-panel" id="tab-utilization" role="tabpanel" style="overflow:hidden; padding: 0 6px 6px 6px;">
                        <!-- Utilization Sub-tab Navigation -->
                        <div class="util-sub-tabs">
                            <button class="util-sub-tab-link active" data-util-tab="daily-hours">Daily Hours</button>
                            <button class="util-sub-tab-link" data-util-tab="project-utilization">Project Utilization</button>
                            <button class="util-sub-tab-link" data-util-tab="team-utilization">Team Member Utilization</button>
                        </div>

                        <!-- Sub-tab: Daily Hours -->
                        <div class="util-sub-tab-panel active" id="util-daily-hours">
                            <div class="util-section-header">
                                <h5>Daily Hours</h5>
                                <span class="util-date-label" id="dailyHoursDateLabel"></span>
                            </div>
                            <div class="util-table-wrapper">
                                <table id="dailyHoursTable" class="matrix-table">
                                    <thead>
                                        <tr id="dailyHoursHeaderRow">
                                            <th>Employee</th>
                                            <!-- Date columns injected by JS -->
                                        </tr>
                                    </thead>
                        ...
                        <!-- Sub-tab: Project Utilization -->
                        <div class="util-sub-tab-panel" id="util-project-utilization">
                            <div class="util-section-header">
                                <h5>Project Utilization</h5>
                            </div>
                            <div class="util-project-layout">
                                <div class="util-chart-box">
                                    <canvas id="projectUtilizationChart"></canvas>
                                </div>
                                <div class="util-project-table-box">
                                    <table id="projectUtilizationTable" class="matrix-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody id="projectUtilizationTableBody">
                                            <!-- Rows injected by JS -->
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <!-- Sub-tab: Team Member Utilization -->
                        <div class="util-sub-tab-panel" id="util-team-utilization">
                            <div class="util-section-header">
                                <h5>Team Member Utilization</h5>
                                <span class="util-date-label" id="teamUtilizationDateLabel"></span>
                            </div>
                            <!-- Color Legend -->
                            <div class="util-legend">
                                <span class="util-legend-item util-red">&#x25A0; &lt;70% Underutilized</span>
                                <span class="util-legend-item util-yellow">&#x25A0; 70–84% Buffer Zone</span>
                                <span class="util-legend-item util-green">&#x25A0; 85–110% Optimal</span>
                                <span class="util-legend-item util-blue">&#x25A0; &gt;110% Overloaded</span>
                                <span class="util-legend-capacity">Capacity: 40 hrs/week</span>
                            </div>
                            <div class="util-table-wrapper">
                                <table id="utilizationMatrixTable" class="matrix-table">
                                    <thead>
                                        <tr id="matrixHeaderRow">
                                            <th>Employee</th>
                                            <!-- Project columns injected by JS -->
                                            <th>Total Hrs</th>
                                            <th>Capacity Hrs</th>
                                            <th>Utilization %</th>
                                        </tr>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function updateRMFilter(selectedDepts) {
                console.log('updateRMFilter run');
            ...
            function updateEmployeeFilter(selectedDepts, selectedRMs) {
                console.log('updateEmployeeFilter run');
                ...
                employeeOptions.forEach(option => {
                    ...
                    const showByRM = selectedRMs.length === 0 || selectedRMs.includes(empRmId) || selectedRMs.includes(empId);
            ...
            function updateAllFilters() {
                console.log('updateAllFilters run');
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            tabs.forEach(tab => {
                tab.addEventListener("click", function (e) {
                    console.log('tab click event listener called');
                    ...
                    const targetName = (currentTab === 'utilization') ? 'tab-utilization' : 'tab-all';
                    const targetPanel = document.getElementById(targetName);
                    if (targetPanel) {
                        targetPanel.classList.add('active');
                    }

                    // Hide graph toggle button on Utilization tab
                    const graphToggleBtn = document.getElementById('toggle-graphs-btn');
                    if (graphToggleBtn) {
                        graphToggleBtn.style.display = (currentTab === 'utilization') ? 'none' : '';
                    }
            ...
            function loadTasks(type, page, limit, filters = {}) {
                ...
                fetch('api/filter-tasks', {
                    ...
                    .then(res => {
                        if (!res.ok) {
                            // Handles HTTP-level errors (e.g., 500, 404)
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success') {
                            if (type === 'utilization') {
                                ...
                                loadUtilizationReports(filters);
                                return;
                            }
                            ...
                            let columns = [];
                            const shortColumns = ['Emp Id', 'Duration', 'Total Time'];
                            const longColumns = ['Description'];
                            switch (type) {
                            ...
                            thead.innerHTML =
                                '<tr><th class="checkbox"><input type="checkbox" id="select-all"></th>' +
                                columns
                                    .map(col => {
                                        const isLong = longColumns.includes(col) ? ' class="long"' : "";
                                        return `<th${isLong}>${col}</th>`;
                                    })
                                    .join("") +
                                "</tr>";
                            ...
                            data.tasks.forEach(task => {
                                ...
                                columns.forEach(col => {
                                    switch (col) {
                                        case "Date": row += `<td class="short">${formatDate(task.date)}</td>`; break;
                                        case "Occasion": row += `<td>${(task.occasion)}</td>`; break;
                                        case "Emp Id": row += `<td class="short">${task.emp_id}</td>`; break;
                                        ...
                                        case "Duration": row += `<td class="short">${formatTime(task.duration) || "-"}</td>`; break;
                                        ...
                                        case "Description": row += `<td class="description-cell long" data-full="${task.description || '-'}">${task.description?.substring(0, 50) || ''}${task.description?.length > 50 ? '...' : ''}</td>`; break;
                            ...
                            // 1. Collect all employees currently VISIBLE in the UI
                            const allVisibleEmployees = Array.from(
                                document.querySelectorAll('.filter-option.employee-filter')
                            ).filter(el => el.closest('.employee-option')?.style.display !== 'none')
                                .map(el => el.value);

                            // 2. If user has manually selected specific employees → use ONLY them
                            let finalEmployees = [];

                            if (filters.selectedEmployees.length > 0) {
                                finalEmployees = filters.selectedEmployees.filter(id =>
                                    allVisibleEmployees.includes(id)
                                );
                                // console.log('case 1');

                            } else {
                                // Else fallback to all visible employees (dept → manager → UI already filtered)
                                finalEmployees = allVisibleEmployees;
                                // console.log('case 2');

                            }

                            // 3. Show or hide graphs based on available employees
                            ...
                            if (finalEmployees.length > 0) {
                                if (!contentArea.classList.contains('graphs-hidden')) {
                                    ...
                                    loadGraphs(finalEmployees);      // Show graphs for ANY number of employees
                                }
                            } else {
                                hideGraphs();
                            }
    ...
    <!-- <script>
        ...
        function loadTasks(caseType, targetPanelId) {
            ...
            fetch("api/filter-tasks", { method: "POST", body: formData })
                .then(res => res.text())
                .then(html => {
