...
//     $taskCategoryIds[] = $taskCategory['cat_id'];
// }
...
<body>
    ...
    <!-- Header removed -->
    <?php
    include_once 'includes/sidebar-executive.php';
    ?>
    <main>
        <div class="top-row">
            <!-- <h3 class="">Home</h3> -->
    ...
    <div id="custom-alert" class="alert-overlay">
        <div class="alert-box" id="alert-box">
            <!-- <h4><i class="fa-solid fa-circle-exclamation"></i>Budget Alert</h4> -->
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function applyFilters() {
                ...
                if (window.location.pathname.includes('dashboard-executive')) {
                    selectedEmployees = [loggedUserId]; // Only one executive
                } else {
                    selectedEmployees = Array.from(document.querySelectorAll('.employee-filter:checked')).map(el => el.value);
                }
                // Get selected departments
                if (window.location.pathname.includes('dashboard-executive')) {
                    selectedDepartments = [];
                } else {
                    selectedDepartments = Array.from(document.querySelectorAll('.department-filter:checked')).map(el => el.value);
                }
                ...
                fetch('api/filter-tasks', {
                    ...
                    .then(data => {
                        ...
                        if (selectedEmployees.length === 1) {
                            //console.log(selectedEmployees[0]);
                            ...
                            if (!contentArea.classList.contains('graphs-hidden')) {
                                ...
                                loadGraphs([selectedEmployees[0]]); // Pass emp_id
                            }
                        } else {
        ...
        document.getElementById('clear-filter-btn')?.addEventListener('click', function () {
            document.querySelectorAll('.filter-option:checked, .timeline-filter:checked, .employee-filter:checked, .status-filter:checked, .department-filter:checked, .rm-filter:checked').forEach(el => {
                el.checked = false;
            });
    ...
    <script>
        function showBudgetPrompt() {
            // Step 1: Ask Yes/No
            const label = (parseInt(deptId) === 5) ? 'Project' : 'Client';
            showAlert(`Is there any budget change in ${label}?`, function () {
        ...
        function showBudgetInput(initialValue = '') {
            ...
            document.getElementById("alert-message").innerHTML = `
                    <div class="form-group">
                        <label for="client-select">Select ${label}</label>
